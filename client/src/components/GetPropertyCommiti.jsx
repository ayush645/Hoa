  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/propertycommiti/update/${selectId}`,
        propertyData
      );
  ...
  const fetchProperty = async (propertyId) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/propertycommiti/get/${propertyId}`
      );
      ...
      if (response.data.success) {
        ...
        setImageData(property.signature || { publicId: "", url: "" });
      } else {
    ...
    <div className="property-info-container p-6 max-h-screen">
      ...
      {showForm && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-50">
          <div className="form bg-gray-100 p-6 rounded-lg shadow-md w-full max-w-4xl relative h-[80vh] overflow-y-scroll">
            <button
            ...
            />
{position === "President" && (
  <div className="w-full flex flex-col items-center bg-gray-100 p-4 rounded-lg shadow-md">
    {/* Signature Display */}
    {imageData?.url && (
      <div className="flex flex-col items-center w-full">
        <p className="text-gray-700 font-semibold mb-2">Signature</p>
        <div className="border border-gray-300 rounded-md p-2 bg-white shadow-sm">
          <img src={imageData.url} alt="Signature" className="w-40 h-auto object-contain" />
        </div>
      </div>
    )}
