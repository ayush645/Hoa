...
const generateOwnersPDF = async (req, res) => {
  try {
    ...
    let owners = []
if(id){
  owners  = await Owner.find({categoryId:id}).populate("categoryId", "name"); // Fetch all owners and populate category name
...
router.get("/generate-pdf", async (req, res) => {
  ...
  try {
    ...
    doc
      .fontSize(18)
      .text(`Payment Details for ${month} ${currentYear}`, 40, 100, {
        align: "center",
      });
